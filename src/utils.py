...
def get_user_search_criteria():
    ...
    while True:
        date_from_str = input(
            "Enter date YYYY-MM-DD from when to look articles from (Optional, press Enter for today): "
        )
...
def build_search_params(criteria: dict):
    ...
    return {"q": criteria["search_term"], "from-date": date_str, "order-by": "newest"}
...
def process_and_print_results(data: dict):
    ...
    if (
        ...
        or "results" not in data["response"]
        or data["response"]["results"] == []
    ):
