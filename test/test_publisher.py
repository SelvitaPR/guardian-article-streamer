...
class TestKinesisPublisher:
    ...
    def test_handles_empty_records_list(self, mocker, stream_name, records):
        ...
        model_instance = KinesisPublisher(
            stream_name=stream_name, region_name="eu-west-2"
        )
    ...
    def test_correct_data_transformation_and_call(
        self, mocker, stream_name, sample_records
    ):
        """
        ...
        mock_kinesis_client.put_records.return_value = {
            "FailedRecordCount": 0,
            "Records": [
                {"SequenceNumber": "1", "ShardId": "0"},
                {"SequenceNumber": "2", "ShardId": "0"},
            ],
            "EncryptionType": "NONE",
        }
        ...
        mock_kinesis_client.put_records.assert_called_once_with(
            Records=expected_records_payload, StreamName=stream_name
        )

    # --- Feature 3: Successful Publish (Simplified check) ---

    def test_successful_publish_prints_success(
        self, mocker, stream_name, sample_records
    ):
        """
    ...
    def test_partial_failure_prints_warning(self, mocker, stream_name, sample_records):
        ...
        mock_print.assert_any_call(
            f"Warning: {FAILED_COUNT} records failed to publish."
        )

    # --- Feature 5: Handling Kinesis Client Exception ---

    def test_kinesis_client_exception_returns_none(
        self, mocker, stream_name, sample_records
    ):
        """
