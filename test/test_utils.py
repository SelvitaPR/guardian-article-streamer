...
class TestGetUserSearchCriteria(unittest.TestCase):
    ...
    def test_invalid_date_format_retries(self, mock_input):
        ...
        self.assertEqual(
            ...
            3,
            "Input function was not called the expected number of times (3).",
        )
        expected_date = datetime.strptime("2024-01-01", "%Y-%m-%d").date()
        self.assertEqual(
            result["date_from"],
            expected_date,
            "Did not return the final, valid date object.",
        )
        self.assertEqual(result["search_term"], "bitcoin")
...
class TestBuildSearchParams(unittest.TestCase):
    ...
    def test_returns_dictionary(self):
        ...
        test_dict = {"search_term": test_search_term, "date_from": test_date_from}
    ...
    def test_non_datetime_input_raises_attribute_error(self):
        ...
        """
        test_dict = {"search_term": "test", "date_from": "a simple string"}
...
class TestProcessAnPrint(unittest.TestCase):
    ...
    TEST_EMPTY_RESPONSE = {"response": {"status": "ok", "total": 0, "results": []}}
    TEST_VALID_RESPONSE = {
        "response": {
            "status": "ok",
            "results": [
